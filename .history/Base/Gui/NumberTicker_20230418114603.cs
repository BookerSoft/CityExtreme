...
namespace CityExtreme.Base.Gui{
    public class NumTicker:Drawable{
        ...
        public NumTicker(Vector2f pos,int _min,int _max,int _stepamt, string desc, int _id){
            ...
            bar.Position =new Vector2f(desclabel.Position.X+desclabel.GetGlobalBounds().Width+20+minuscir.GetGlobalBounds().Width,minuscir.GetGlobalBounds().minuscir.GetGlobalBounds().Height/2);
