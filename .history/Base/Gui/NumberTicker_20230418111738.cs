...
namespace CityExtreme.Base.Gui{
    public class NumTicker:Drawable{
        ...
        public NumTicker(Vector2f pos,int _min,int _max,int _stepamt, string desc, int _id){
            ...
            minus.Position = new Vector2f(bounds.Left + minuscir.GetGlobalBounds().Left+(minuscir.GetGlobalBounds().Width*.2f),bounds.Top + (minuscir.GetGlobalBounds().Height*.4f));
            minus.FillColor = Color.White;
            minus.Size = new Vector2f((minuscir.GetGlobalBounds().Width*.8f)-(minuscir.GetGlobalBounds().Width*.2f),minuscir.GetGlobalBounds().Height*.1f);
            ...
            cstepcir.Position= new Vector2f(bar.Position.X+((float)step*(float)cstepciroffset),bounds.Top + bar.Position.Y + (bar.GetGlobalBounds().Height/2)); 
        }
