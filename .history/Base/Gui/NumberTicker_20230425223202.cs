...
namespace CityExtreme.Base.Gui{
    public class NumTicker:Drawable{
        ...
        public void Click(object? sender, MouseButtonEventArgs e){
            if(e.Button== Mouse.Button.Left){
                if(minuscir.GetGlobalBounds().Contains(e.X,e.Y)){
                    ...
                    if(isIntTicker){
                       step++;
                       int num = ((int)min+((int)step-1)*(int)stepamt);
                       numlabel.DisplayedString = num.ToString();
                       PreGame_Menu.PreLobby_FE.numTickerNLDS[0] = numlabel.DisplayedString;
        ...
        public void Draw(RenderTarget target, RenderStates states)
        {
            ...
            if(numlblupdneeded)
            {
                numlabel.DisplayedString = PreGame_Menu.PreLobby_FE.numTickerNLDS[0];
