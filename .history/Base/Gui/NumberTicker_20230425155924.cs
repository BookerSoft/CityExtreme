...
namespace CityExtreme.Base.Gui{
    public class NumTicker:Drawable{
        ...
        public bool isIntTicker,isDoubleTicker;
        ...
        public void Click(object? sender, MouseButtonEventArgs e){
            if(e.Button== Mouse.Button.Left){
                ...
                }else if(((IntRect)pluscir.GetGlobalBounds()).Contains(e.X,e.Y)){
                    if(isIntTicker && !isDoubleTicker){
                       step++;
                       numlabel.DisplayedString = min+((step-1)*stepamt).ToString();
